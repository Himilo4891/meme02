...
extension TableViewController: UICollectionViewDelegate, UITableViewDataSource {
    ...
    func editMemeViewController(segue: UIStoryboardSegue) -> ViewController {
          ...
          func TableViewController(segue: UIStoryboardSegue) -> ViewController {
              let viewController = segue.destination as! TableViewController
              return TableViewController(segue: segue)
          }

          
              func prepare(for segue: UIStoryboardSegue, sender: Any?) {
              if "goToEditor" == segue.identifier{
                  _ = TableViewController(segue: segue)
//                  TableViewController.memes = self.getMemes()
//              } else if "goToDetails" == segue.identifier {
//                  let viewController = TableViewController(segue: segue)
//                  TableViewController.memes = self.getMemes
              }
          }
      }
